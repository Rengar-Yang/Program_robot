...
#endif 
int Usart_Receive;
static unsigned char TxBuffer[256];//控制舵机用的数组
...
//        else if(Encoder_C<0) Direction_D=2;
//	      else                 Direction_D=1;
...
//{
//	unsigned char i = 0;
//	for (i=0;i<len;i++) UART3_Put_Char(*(Str + i));
//}

void UART_DM_ReportData(unsigned char data[],unsigned char len)
{		
	//UART3_Put_StringL(data,10);     //打印长度根据所打印的数组长度来改，动作组是5，速度状态是10。最好填10
	unsigned char i = 0;
	for (i=0;i<len;i++) 
	usart3_send(data[i]); //帧头	
}
